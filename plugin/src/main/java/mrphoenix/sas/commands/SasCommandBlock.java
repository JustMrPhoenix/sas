...
public class SasCommandBlock extends SasCommandBase {
    ...
    public SasCommandBlock(SasPlugin plugin) {
        super(plugin, Arrays.asList("sastimeout", "sasuntimeout"));
        if (plugin.getDataBaseConfig().getFileConfiguration().contains("blocks")) {
            config = plugin.getDataBaseConfig().getSection("blocks");
    ...
    public void save() {
        ...
        if (map != null) {
            plugin.getDataBaseConfig().set("blocks", map);
            plugin.getDataBaseConfig().save();
        }
    }
